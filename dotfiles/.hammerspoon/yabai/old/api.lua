...
function M.space.get_space_with_highest_id_of_display(display_index, callback)
	M.space.get_by_display(display_index, function(error, spaces)
		if not utils.is_empty(error) then
			ensure(callback, error, nil)
		else
			local highest_space_id
			local found_space

			for _, space in ipairs(spaces) do
				if highest_space_id == nil then
					highest_space_id = space.id
					found_space = space
				else
					if space.id > highest_space_id then
						highest_space_id = space.id
						found_space = space
					end
				end
			...
			ensure(callback, nil, found_space)
		end
	end)
end
...
function M.space.create(space_label, display_index, callback)
	...
	yabai({ "-m", "space", "--create" }, function(_, error)
		...
		M.space.get_space_with_highest_id_of_display(display_index, function(space_with_highest_id_error, space)
			if not utils.is_empty(space_with_highest_id_error) then
				ensure(callback, space_with_highest_id_error, nil)
			else
				if space ~= nil then
					...
					local verify_callbacks = utils.aggregate_callbacks(callbacks_count, function(aggregate_error, _)
						ensure(callback, aggregate_error, space)
					end)

					if must_move then
						M.space.move(space.index, display_index, function(move_error)
					...
					if must_label then
						M.space.label(space.index, space_label, function(label_error)
					...
					if not utils.is_empty(create_error) then
						...
						M.window.move(window.id, space.index, function(move_error)
							if not utils.is_empty(move_error) then
								...
								M.space.focus(space.index, function(focus_error)
									if not utils.is_empty(focus_error) then
										ensure(callback, focus_error, nil)
									else
										ensure(callback, nil, nil)
									end
								end)
