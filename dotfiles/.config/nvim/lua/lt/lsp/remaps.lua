...
function M.set_default_on_buffer(client, bufnr)
  ...
  if cap.definitionProvider then
    ...
    buf_set_keymap('n', 'gd', '<cmd>lua vim.lsp.buf.definition()<CR>', 'lsp',
                   'lsp_preview_definition', 'Preview definition')
  end
  ...
  if cap.referencesProvider then
    ...
    buf_set_keymap('n','gR', "<cmd>lua require('telescope.builtin').lsp_references()<CR>", 'lsp', 'lsp_references', 'Show references')
  ...
  if cap.documentSymbolProvider then
    ...
    buf_set_keymap('n', '<leader>tO',
                   '<cmd>lua require(\'telescope.builtin\').lsp_document_symbols()<CR>',
                   'lsp', 'lsp_document_symbols', 'Document symbols')
    buf_set_keymap('n', '<leader>to', '<cmd>Aerial!<CR>', 'lsp',
  ...
  buf_set_keymap('n', '<leader>fE',
                 ...
                 'lsp', 'lsp_show_line_diagnostics', 'Show line diagnostics')

  buf_set_keymap('n', '[e', '<cmd>lua vim.diagnostic.goto_prev()<CR>', 'lsp',
                 'lsp_previous_diagnostic', 'Previous diagnostic')
  buf_set_keymap('n', ']e', '<cmd>lua vim.diagnostic.goto_next()<CR>', 'lsp',
                 'lsp_next_diagnostic', 'next diagnostic')

  if cap.documentFormattingProvider then
    buf_set_keymap('n', '<leader>ff', '<cmd>lua vim.lsp.buf.formatting()<CR>',
                   'lsp', 'lsp_format', 'Format')
  elseif cap.documentRangeFormattingProvider then
    buf_set_keymap('n', '<leader>ff', '<cmd>lua vim.lsp.buf.formatting()<CR>',
                   'lsp', 'lsp_format_2', 'Format')
  end

  if cap.renameProvider then
    -- buf_set_keymap('n','<leader>rr','<cmd>lua vim.lsp.buf.rename()<CR>', opts)
    -- buf_set_keymap('n','<leader>rr', "<cmd>lua require('lspsaga.rename').rename()<CR>", 'lsp', 'lsp_rename', 'Rename')
    buf_set_keymap('n', '<leader>rr',
                   '<cmd>lua require(\'renamer\').rename({ empty = true })<cr>',
                   'lsp', 'lsp_rename_empty', 'Rename')
    buf_set_keymap('n', '<leader>rR',
                   '<cmd>lua require(\'renamer\').rename({ empty = false })<cr>',
                   'lsp', 'lsp_rename', 'Rename with existing value')
  end

  buf_set_keymap('n', '<leader>flc',
                 ':lua print(vim.inspect(vim.lsp.get_active_clients()))<CR>',
                 'lsp', 'lsp_debug_clients', '[DEBUG] LSP clients')
  buf_set_keymap('n', '<leader>fll', ':lua print(vim.lsp.get_log_path())<CR>',
